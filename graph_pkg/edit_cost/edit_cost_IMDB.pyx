cdef class EditCostIMDB(EditCost):
    ...
    cdef double c_cost_substitute_node(self, Node node_src, Node node_trgt):
        self.actor_source = node_src.label.actor
        self.actor_target = node_trgt.label.actor

        return self.alpha_node * self.metric(self.actor_source, self.actor_target) * (self.c_insert_node + self.c_delete_node)
